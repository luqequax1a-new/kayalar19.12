<div class="my-orders">
    <div class="my-orders-cards">
        @foreach ($orders as $order)
            <div class="my-order-card">
                <div class="my-order-card-header">
                    <div class="my-order-card-title">
                        <div class="my-order-card-label">{{ trans('storefront::account.orders.order_id') }}</div>
                        <div class="my-order-card-value">{{ $order->displayOrderNumber() }}</div>
                    </div>

                    <div class="my-order-card-status">
                        <div class="my-order-card-label">{{ trans('storefront::account.status') }}</div>
                        <span class="badge {{ order_status_badge_class($order->status) }}">{{ $order->status() }}</span>
                    </div>
                </div>

                <div class="my-order-card-body">
                    <div class="my-order-card-row">
                        <div class="my-order-card-row-label">{{ trans('storefront::account.date') }}</div>
                        <div class="my-order-card-row-value">{{ $order->created_at->format('d.m.Y') }}</div>
                    </div>

                    <div class="my-order-card-row">
                        <div class="my-order-card-row-label">{{ trans('storefront::account.view_order.payment_method') }}</div>
                        <div class="my-order-card-row-value">{{ $order->payment_method }}</div>
                    </div>

                    <div class="my-order-card-row">
                        <div class="my-order-card-row-label">{{ trans('storefront::account.orders.total') }}</div>
                        <div class="my-order-card-row-value my-order-card-row-value--strong">{{ $order->total->convert($order->currency, $order->currency_rate)->format($order->currency) }}</div>
                    </div>
                </div>

                <div class="my-order-card-footer">
                    <a href="{{ route('account.orders.show', $order) }}" class="btn my-order-card-btn">
    ...
    <div class="table-responsive my-orders-table-wrap">
