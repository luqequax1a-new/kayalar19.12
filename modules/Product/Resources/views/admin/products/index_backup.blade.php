    ...
    @slot('filters')
        <div class="products-page-top">
            <div class="products-page-topbar">
                <div class="products-page-title">{{ trans('product::products.products') }}</div>
                <div id="products-actions-host" class="products-page-actions"></div>
            </div>

            <form id="product-filters" class="form-inline product-filters">
            <div class="product-stats">
                <div class="product-stats-grid">
                    <div class="product-stat-card">
                        <div class="product-stat-icon"><i class="fa fa-cubes"></i></div>
                        <div class="product-stat-body">
                            <div class="product-stat-label">Toplam</div>
                            <div id="product-stat-total" class="product-stat-value">{{ $stats['total'] ?? '-' }}</div>
                        </div>
                    </div>
                    <div class="product-stat-card">
                        <div class="product-stat-icon product-stat-icon-success"><i class="fa fa-check-circle"></i></div>
                        <div class="product-stat-body">
                            <div class="product-stat-label">Aktif</div>
                            <div id="product-stat-active" class="product-stat-value">{{ $stats['active'] ?? '-' }}</div>
                        </div>
                    </div>
                    <div class="product-stat-card">
                        <div class="product-stat-icon product-stat-icon-muted"><i class="fa fa-pause-circle"></i></div>
                        <div class="product-stat-body">
                            <div class="product-stat-label">Pasif</div>
                            <div id="product-stat-inactive" class="product-stat-value">{{ $stats['inactive'] ?? '-' }}</div>
                        </div>
                    </div>
                    <div class="product-stat-card">
                        <div class="product-stat-icon product-stat-icon-success"><i class="fa fa-archive"></i></div>
                        <div class="product-stat-body">
                            <div class="product-stat-label">Stokta</div>
                            <div id="product-stat-in-stock" class="product-stat-value">{{ $stats['in_stock'] ?? '-' }}</div>
                        </div>
                    </div>
                    <div class="product-stat-card">
                        <div class="product-stat-icon product-stat-icon-danger"><i class="fa fa-times-circle"></i></div>
                        <div class="product-stat-body">
                            <div class="product-stat-label">Stok Yok</div>
                            <div id="product-stat-out-of-stock" class="product-stat-value">{{ $stats['out_of_stock'] ?? '-' }}</div>
                        </div>
                    </div>
                    <div class="product-stat-card">
                        <div class="product-stat-icon product-stat-icon-warning"><i class="fa fa-exclamation-triangle"></i></div>
                        <div class="product-stat-body">
                            <div class="product-stat-label">Düşük Stok</div>
                            <div id="product-stat-low-stock" class="product-stat-value">{{ $stats['low_stock'] ?? '-' }}</div>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="form-group">
                ...
                <select name="stock" id="filter-stock" class="form-control input-sm">
                    <option value="">Tümü</option>
            ...
            <button type="button" id="btn-clear-filters" class="btn btn-default btn-sm">
                Temizle
            </button>
            ...
            <div id="products-dt-controls" class="dt-controls-inline">
                <div class="dt-controls-left"></div>
                <div class="dt-controls-right"></div>
            </div>
            </div>
            </form>
        </div>
...
@endif
    ...
    <script type="module">
        ...
        function moveDtControlsInline() {
            const target = document.getElementById('products-dt-controls');
            if (!target) return;

            const left = target.querySelector('.dt-controls-left');
            const right = target.querySelector('.dt-controls-right');
            if (!left || !right) return;

            const wrapper = document.querySelector('#products-table .dataTables_wrapper');
            if (!wrapper) return;

            // DataTables 2 layout
            const layoutRow = wrapper.querySelector('.dt-layout-row');
            if (layoutRow) {
                if (!right.contains(layoutRow)) {
                    right.appendChild(layoutRow);
                }

                layoutRow.classList.add('products-dt-toolbar');
            }

            // DataTables 1.x legacy bootstrap layout
            const legacyLength = wrapper.querySelector('.dataTables_length');
            const legacyFilter = wrapper.querySelector('.dataTables_filter');

            if (legacyLength && !left.contains(legacyLength)) {
                left.appendChild(legacyLength);
                legacyLength.classList.add('products-dt-legacy');
            }

            if (legacyFilter && !right.contains(legacyFilter)) {
                right.appendChild(legacyFilter);
                legacyFilter.classList.add('products-dt-legacy');
            }
        }
        ...
        function setStats(stats) {
            if (!stats) return;
            const map = {
                total: '#product-stat-total',
                active: '#product-stat-active',
                inactive: '#product-stat-inactive',
                in_stock: '#product-stat-in-stock',
                out_of_stock: '#product-stat-out-of-stock',
                low_stock: '#product-stat-low-stock',
            };
        ...
        function loadStats() {
            ...
            axios
                .get('{{ route('admin.products.stats') }}', {
    ...
    <style>
        ...
        .index-table-actions .btn,
        .index-table-actions .btn-group > .btn {
            border-radius: 10px;
        }

        .index-table-actions .btn-actions {
            height: 34px;
            padding: 6px 12px;
            display: inline-flex;
            align-items: center;
        ...
        .index-table-actions .dropdown-menu {
            border-radius: 12px;
            padding: 6px 0;
        }
        ...
        .products-page-actions .index-table-actions-row {
            width: auto;
            margin: 0;
        }

        .products-page-actions .index-table-actions {
            gap: 10px;
        }

        .products-page-actions .btn-actions,
        .products-page-actions .btn-group > .btn {
            border-radius: 999px;
            height: 38px;
            padding: 8px 14px;
            font-weight: 700;
        ...
        .products-page-actions .btn-actions i,
        .products-page-actions .btn-group > .btn i {
            width: 22px;
            height: 22px;
            border-radius: 999px;
            display: inline-flex;
        ...
        .products-page-actions .btn-primary {
            background: linear-gradient(180deg, #2563eb 0%, #1d4ed8 100%);
            border-color: rgba(37, 99, 235, 0.35);
        }

        .products-page-actions .btn-success {
            background: linear-gradient(180deg, #22c55e 0%, #16a34a 100%);
            border-color: rgba(34, 197, 94, 0.35);
        }

        .products-page-actions .btn-default {
            background: #ffffff;
            color: #0f172a;
        }
        ...
        .product-stat-icon-success {
            background: #ecfdf5;
            color: #047857;
        }

        .product-stat-icon-danger {
            background: #fef2f2;
            color: #b91c1c;
        }

        .product-stat-icon-warning {
            background: #fffbeb;
            color: #b45309;
        }

        .product-stat-icon-muted {
            background: #f3f4f6;
            color: #6b7280;
        }
        ...
        .product-filters-bar .form-group {
            margin: 0;
        }
